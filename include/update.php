...
$terminos = isset($_POST["TerminosYCondiciones"]) ? 1 : 0;

// Validaciones
if (empty($documento) || empty($Nombre) || empty($email) || empty($telefono) || empty($mensaje) || !$terminos) {
...
$stmt = $mysql->prepare("UPDATE cliente SET Nombre = ?, Correo = ?, Telefono = ?, FechaNacimiento = ?, Genero = ?, Mensaje = ?, TerminosYCondiciones = ? WHERE Documento = ?");
if ($stmt) {
    // Enlazar parámetros
    $stmt->bind_param("ssssssis", $Nombre, $email, $telefono, $fecha, $genero, $mensaje, $terminos, $documento);
